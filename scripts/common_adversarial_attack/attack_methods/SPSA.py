...
class SPSAAttack():
    def __init__(self, model, attack_setting='score', search='orig'):
        ...
        if self.model.kwargs['vision_model'] == 'ViT-B/32':
            self.prob = torch.zeros(self.q, 3, 384, 384).fill_(0.5)
        elif self.model.kwargs['clip_type'] == 'RN50x4':
            self.prob = torch.zeros(self.q, 3, 288, 288).fill_(0.5)
        else:
            self.prob = torch.zeros(self.q, 3, 224, 224).fill_(0.5)
    ...
    def compute_grad(self, images, target_label):
        with torch.no_grad():
            sigma = 0.001
            g = 0
            u = (torch.bernoulli(self.prob) * 2 - 1).to(self.device)
            logits1 = self.model(images + sigma * u)  # self.q, 1000
            tmp1 = logits1[:, target_label] - \
                   torch.max(torch.cat((logits1[:, :target_label], logits1[:, target_label + 1:]), dim=1), dim=1)[0]
            logits2 = self.model(images - sigma * u)
            tmp2 = logits2[:, target_label] - \
                   torch.max(torch.cat((logits2[:, :target_label], logits2[:, target_label + 1:]), dim=1), dim=1)[0]
