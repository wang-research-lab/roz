...
class DIMAttack():
    ...
    def img_transform(self, img):
        if np.random.uniform() > self.p:
            ...
            rnd = np.random.randint(int(0.9 * img_size), img_size + 1)  # 201, 224
            resize_img = resize(img, rnd)  # 3, rnd, rnd   # 3, 211, 211
            left = img_size - rnd  # 13
            if left % 2 == 0:
                pad_img = pad(resize_img, int(left / 2))
            else:
                pad_img = pad(resize_img, [left // 2, left // 2, left // 2 + 1, left // 2 + 1])
            return pad_img
        else:
            return img
    ...
    def iterate_attack(self, images, labels, epsilon):
        ...
        for i in range(iter - 1):
            ...
            if self.attack_setting == 1:
                output = self.substitue_model(self.img_transform(pert_out))
            else:
                output = self.model(self.img_transform(pert_out))
