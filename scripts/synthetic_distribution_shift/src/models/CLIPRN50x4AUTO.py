...
def classifier_loader():
    model, _ = clip.load("RN50x4", jit=False)
    return model
...
def construct_class_texts_AUTO(class_name, stack_prompt_tensor):
    ...
    class_texts = clip.tokenize(class_texts)  # 23, 77
    # (23, 1) (23, 4) (23, ())
...
def preprocess(model):
    prompts_li = get_promptsid_li('/home/chenyangyi/RobustVisionFeatures/AutoPrompt/RN50x4file.log')
    sorted_prompts_li = sorted(prompts_li.items(), key=lambda item: item[1], reverse=True)

    # print('template number: ', len(sorted_prompts_li))
    promptids_li = [item[0] for item in sorted_prompts_li[:7]]
...
def snippet(images, model):
    global flag, zeroshot_weights
    if flag:
        zeroshot_weights = preprocess(model)
        flag = False
    image_features = model.encode_image(images)  # 32, 512
    # print(image_features.shape)
    image_features /= image_features.norm(dim=-1, keepdim=True)
    logits = 100. * image_features @ zeroshot_weights  # 32, 1000
    return logits.float()
...
registry.add_model(
    Model(
        name = 'CLIPRN50x4AUTO',
        transform =  StandardTransform(img_resize_size=288, img_crop_size=288),
        normalization = StandardNormalization(mean=[0.48145466, 0.4578275, 0.40821073], std=[0.26862954, 0.26130258, 0.27577711]),
        classifier_loader = classifier_loader,
        eval_batch_size = 64,

        classify = lambda images, model: snippet(images, model=model)
    )
)